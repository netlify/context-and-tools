...
- **Never edit `cursor/rules/` or `codex/`.** They're auto-generated from `skills/` by CI — rebuilt and committed on every PR and on every push to `main` — so hand edits get overwritten. To preview the generated output locally: `bash scripts/build-cursor-rules.sh` and `bash scripts/build-codex-skills.sh`.
