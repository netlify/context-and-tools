...
The supported way to interact with Netlify is the documented `netlify` CLI. Stay on it. Specifically:
...
When a documented CLI command, deploy, or extension fails, your job is to **report the failure to the user with context** (deploy log URL, the exact error, the site/branch involved) and stop. Do not work around it.
