...
5. Provision the production database, apply schema, import data (cutover).
...
Remove the old provider's packages and install `@netlify/database`:
...
Replace the existing Drizzle client setup. `getDatabase()` returns different drivers depending on the runtime (`NETLIFY_DB_DRIVER`, set by the platform):
...
Note: `getDatabase()` throws immediately if `NETLIFY_DB_URL` is not set. This is different from some providers' lazy clients — any code that imports the db module without the env var will fail at import time, not at query time.
...
export default defineConfig({
  dialect: 'postgresql',
  schema: './path/to/schema.ts',          // keep existing path
  out: './netlify/db/migrations',          // GA convention
...
```bash
git mv migrations netlify/db/migrations   # or wherever they currently live
```
...
If the lexicographic sort of `netlify/db/migrations/*.sql` does not match the `idx` order in `_journal.json`, **rename the offending files** to timestamp prefixes using the `when` values from `_journal.json`:

```bash
date -u -r <unix_seconds> +%Y%m%d%H%M%S   # convert
git mv netlify/db/migrations/<old>_<name>.sql netlify/db/migrations/<new>_<name>.sql
git mv netlify/db/migrations/meta/<old>_snapshot.json netlify/db/migrations/meta/<new>_snapshot.json
...
Once the production Netlify Database is provisioned (either by merging the branch or during a parallel-packages transition), apply schema and import data:

```bash
# Apply schema if not yet auto-applied by a deploy
