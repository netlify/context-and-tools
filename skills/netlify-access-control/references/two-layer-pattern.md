...
// Returns autoconfirm, disableSignup, and providers — read this in the deployed app
const settings = await getSettings()
```

`getSettings()` only works against a deployed site (Identity doesn't run under `netlify dev`), so it helps the running app render the right buttons — it does not help at authoring time. Before scaffolding, ask the user what's already configured rather than guessing, and hand off any dashboard changes with an explicit checklist.
