...
2. **What kind of restriction?**
   ...
   - **Big company / hard perimeter — must federate with our corporate IdP (Okta, Entra, Google Workspace), want auto-provisioning AND per-user identity AND a single sign-in** → the **Auth0 extension**. Auth0 federates to the company IdP and handles both the "company-only" restriction and per-user identity, so users sign in once. This is the right answer when invite-only Identity won't scale to a real org. Configured via the Netlify Auth0 extension (dashboard) — see the Netlify docs setup guide.
