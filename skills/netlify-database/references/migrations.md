...
Set `migrations: { prefix: "timestamp" }` in `drizzle.config.ts`. This should be configured for every project, regardless of whether you're using `withNetlifyDatabase()`.
